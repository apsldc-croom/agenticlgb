...
import { PMService, PMDashboard, PMPhase, PMFeature, PMArchLayer, PMDocEntry, PMTechDebt, PMDeployment, PMBusinessGoal } from '@/lib/api/pm';
            ...
            <div className="relative z-10 max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
                ...
                <div className="flex items-center justify-between mb-8 flex-wrap gap-4">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-slate-500 text-sm">Elitie.in &middot; 26-Layer Architecture Roadmap</p>
                        </div>
                    </div>
                    <div className="flex gap-3">
                        <Link href="/dashboard"><button className="px-4 py-2 rounded-xl text-sm text-slate-400 border border-white/10 hover:bg-white/[0.05] transition-all">Back</button></Link>
                        <Link href="/pm/tasks"><button className="px-5 py-2.5 rounded-xl text-sm font-medium text-white bg-gradient-to-r from-cyan-500 to-indigo-600 shadow-lg shadow-cyan-500/20">All Tasks</button></Link>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
                    <div className="lg:col-span-2 rounded-2xl border border-white/[0.06] bg-white/[0.02] p-6">
                        <div className="flex items-start gap-8 flex-wrap">
                            <Ring value={dash.completion_percentage} />
                            ...
                            {deploys.map(d => (
                                <div key={d.id} className="flex items-center justify-between">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-sm text-slate-300">{d.environment_display}</span>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <span className="text-[10px] text-slate-500">{d.status_display}</span>
                ...
                <Section title="Execution Phases" icon={GitBranch} count={phases.length}>
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                        {phases.map(p => (
                            <Link href={`/pm/tasks?phase=${p.id}`} key={p.id}>
                                    ...
                                    <div className="flex items-start justify-between mb-3">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-[11px] text-slate-500">{p.duration_weeks}w &middot; {p.task_count} tasks</p>
                                            </div>
                                        </div>
                                ...
                                <div key={f.id} className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-4 flex items-center gap-3">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <div className="flex items-center gap-2 mt-0.5">
                                            <Pill status={f.status} label={f.status_display} />
                        ...
                        <div className="space-y-1.5 max-h-80 overflow-y-auto pr-1">
                            {docs.map(d => (
                                <div key={d.id} className="flex items-center gap-3 rounded-lg bg-white/[0.02] border border-white/[0.04] px-3 py-2">
                                    <span className="text-[10px] text-slate-500 w-20 shrink-0 capitalize">{d.doc_type_display}</span>
                                    ...
                                    <Pill status={d.status} label={d.status_display} />
                                </div>
                            ))}
                        </div>
                    </div>
                ...
                <div className="text-center pb-4"><p className="text-xs text-slate-600">9 Phases &middot; {dash.total_tasks} Tasks &middot; {features.length} Features &middot; {layers.length} Arch Layers &middot; {docs.length} Docs &middot; {debts.length} Debts &middot; {goals.length} OKRs</p></div>
            </div>
        </div>
    );
}